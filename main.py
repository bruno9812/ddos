...
while 1: #Infinitely loops sending packets to the port until the program is exitede
			sent= sent + 1 #adding one to the sent number
			port= port + 1 #adding one to the port numbe
			print ("Sent %s amount of packets to %s at port 80." % (sent,ip,port)) #printing how many times it has sent it and what port it sent to.
			if port > 65000:
				port = 1